...
Scheme_Object *scheme_hash_get_pair(Scheme_Hash_Table *table, Scheme_Object *key)
{
  Scheme_Object *interned_key, *val;

  val = scheme_hash_get_w_key_wraps(table, key, NULL, &interned_key);

  if (val == NULL)
    return NULL;

  return scheme_make_pair(interned_key, val);
}
                                    ...
                                    Scheme_Object *key_wraps, GC_CAN_IGNORE Scheme_Object **_interned_key)
{
  ...
  if (bucket) {
    if (_interned_key) *_interned_key = (Scheme_Object *)bucket->key;
    return bucket->val;
  } else {
    return NULL;
}
...
Scheme_Object *
scheme_lookup_pair_in_table (Scheme_Bucket_Table *table, const char *key)
{
  Scheme_Object *interned_key, *val;

  val = (Scheme_Object *)scheme_lookup_in_table_w_key_wraps(table, key, NULL, &interned_key);

  if (val) {
    if (table->weak)
      interned_key = (Scheme_Object *)HT_EXTRACT_WEAK(interned_key);
...
Scheme_Object *scheme_hash_tree_get_pair(Scheme_Hash_Tree *tree, Scheme_Object *key)
{
  Scheme_Object *interned_key, *val;

  val = scheme_hash_tree_get_w_key_wraps(tree, key, NULL, &interned_key);
